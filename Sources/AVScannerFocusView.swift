...
public class AVScannerFocusView: UIView {
    ...
    private enum Config {
        ...
        static let breathAnimationOriginSize = CGSize(width: 200, height: 200)
        static let breathAnimationTargetSize = CGSize(width: 275, height: 125)
        // View configuration
        static let borderColor: UIColor = .white
        static let borderWidth: CGFloat = 1.5
        static let centerLineLength: CGFloat = 5.0
    }
...
extension AVScannerFocusView {
    func startAnimation() {
        ...
        UIView.animate(withDuration: 0.1, animations: {
            self.alpha = 0
        }, completion: { [superview] _ in
            self.layer.transform = CATransform3DIdentity
            self.layer.removeAllAnimations()
            
            self.frame.size = Config.breathAnimationOriginSize
            self.center = CGPoint(
                x: superview.center.x - Config.breathAnimationOriginSize.width / 2,
                y: superview.center.y - Config.breathAnimationOriginSize.height / 2 - superview.frame.minY
            )
            
            let targetX = superview.center.x - Config.breathAnimationTargetSize.width / 2
            let targetY = superview.center.y - Config.breathAnimationTargetSize.height / 2 - superview.frame.minY
            ...
            UIView.animate(withDuration: 1.25, delay: 0, options: [.autoreverse, .repeat, .curveEaseInOut], animations: {
                self.frame = CGRect(origin: targetOrigin, size: Config.breathAnimationTargetSize)
            }, completion: nil)
        })
    }
...
fileprivate extension AVScannerFocusView {
    func transformTo(topLeft tl: CGPoint, bottomLeft bl: CGPoint, bottomRight br: CGPoint, topRight tr: CGPoint, completion: (() -> Void)? = nil) {
        guard layer.anchorPoint.equalTo(CGPoint.zero) else { return }
        
        let b = boundingBoxForQuadTR(topLeft: tl, topRight: tr, bottomLeft: bl, bottomRight: br)
        ...
        UIView.animate(withDuration: 0.3, delay: 0, options: [.curveEaseInOut, .beginFromCurrentState], animations: {
            self.frame = b
            self.layer.transform = self.rectToQuad(
                rect: self.bounds,
    ...
    func boundingBoxForQuadTR(topLeft tl: CGPoint, topRight tr: CGPoint, bottomLeft bl: CGPoint, bottomRight br: CGPoint) -> CGRect {
        ...
        let minX: CGFloat = min(min(min(tr.x, tl.x), bl.x), br.x)
        let minY: CGFloat = min(min(min(tr.y, tl.y), bl.y), br.y)
        let maxX: CGFloat = max(max(max(tr.x, tl.x), bl.x), br.x)
        let maxY: CGFloat = max(max(max(tr.y, tl.y), bl.y), br.y)
        
        b.origin.x = minX
        b.origin.y = minY
        b.size.width = maxX - minX
        b.size.height = maxY - minY
